...
/**
* Sphere同士の衝突判定
* @param _sphereA --- 判定する図形（１）
* @param _sphereB --- 判定する図形（２）
* @param inter    --- 衝突点を格納する変数への参照
*/
...
bool CheckSphere2Sphere(const Sphere& _sphereA, const Sphere& _sphereB,Vector3* inter)
{
	...
	if (inter)
	{
		// 球A、球Bの中心座標を半径の比率で内分した点を疑似交点とする
...
bool CheckCapsule2Capsule(const Capsule& _capsuleA, const Capsule& _capsuleB)
{
	...
	// 距離（の二乗）が半径の和（二乗）より大きければ当たっていない
...
* @param _segment0 --- 線分0
* @param _segment1 --- 線分1
* @param _closest0 --- 線分0上の最近接点(結果出力用）
* @param _closest1 --- 線分1上の最近接点(結果出力用）
*/
